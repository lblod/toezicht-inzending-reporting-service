...
const generateCsvReportYesterday = async function(filenameBase) {
  let yesterday = new Date(Date.now() - 86400000); // 24 * 60 * 60 * 1000
  yesterday.setHours(0, 0, 0, 0);
  let today = new Date(Date.now());
  today.setHours(0, 0, 0, 0);

  const file = `${outputDir}/${filenameBase}-gisteren.csv`;
  let query = await fs.readFile('/app/sparql-queries/inzendingen-gisteren.sparql', 'utf8');
  query = query.replace('{YESTERDAY}', yesterday.toISOString());
  query = query.replace('{TODAY}', today.toISOString());
  await queryCsv(query, file);
};

const generateCsvReportForCurrentPeriod = async function() {
  // TODO execute select query to get huidige periode's inzendingen
};
...
class ReportTask {
  ...
  async perform() {
    ...
    try {
      const filename = `${fileBase}-${new Date().toISOString().replace(/-|T|Z|:|\./g, '')}`;

      await generateCsvReportYesterday(filename);
