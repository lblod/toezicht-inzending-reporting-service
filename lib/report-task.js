...
const lineTransform = function(values) {
  // Note: order of the values depends on the order of the variables in the SPARQL SELECT statement
  const besluitDatum = values[7];
  if (besluitDatum)
    values[7] = moment(besluitDatum).format('DD-MM-YYYY');

  const ontvangstDatum = values[8];
  if (ontvangstDatum)
    values[8] = moment(ontvangstDatum).format('DD-MM-YYYY HH:mm:ss');

  let loketUrls = [];
  const loketUrlsString = values[13];
  if (loketUrlsString.length >= 2) // remove start and end " before splitting on ';'
    loketUrls = loketUrlsString.substring(1, loketUrlsString.length -1).split(';').map(u => `"${u}"`);

  // Spread loket URLs over multiple columns so there is only 1 URL per field
  const columns = values.slice(0, 13).concat(loketUrls);
  return columns;
};
